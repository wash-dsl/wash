...
double SedovComputer::sedovSol(const size_t dim, const double time, const double eblast, const double omega_i,
                               ...
                               const double vel0, const double cs0, const std::vector<double>& r, std::vector<double>& rho,
                               std::vector<double>& p, std::vector<double>& u, std::vector<double>& vel, std::vector<double>& cs)
{
    ...
    std::function<void(const size_t, std::function<double(const double)>, const double, const double, double&)> choose)
{
    ...
    for (size_t i = 0; i < imax; i++)
    {
        ...
        if (i >= j - 1)
        {
            ...
            if (std::abs(dss) <= eps * std::abs(ss)) return;
        }
...
double SedovComputer::zeroin(const double ax, const double bx, std::function<double(const double)> f, const double tol)
{
