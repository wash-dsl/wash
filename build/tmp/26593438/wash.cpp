...
namespace wash {
    ...
    void set_default_neighbor_search(const unsigned max_count) {
        ...
        neighbors_kernel = [](Particle& p) { wash::recalculate_neighbours(p,neighbors_max); };
    }
...
;
    ...
    void start() {
        ...
        wash::scalar_force_nc.resize(local_count);
wash::scalar_force_temp.resize(local_count);
wash::scalar_force_p.resize(local_count);
wash::scalar_force_c.resize(local_count);
wash::scalar_force_c11.resize(local_count);
wash::scalar_force_c12.resize(local_count);
wash::scalar_force_c13.resize(local_count);
wash::scalar_force_c22.resize(local_count);
wash::scalar_force_c23.resize(local_count);
wash::scalar_force_c33.resize(local_count);
wash::scalar_force_du.resize(local_count);
wash::scalar_force_du_m1.resize(local_count);
wash::scalar_force_dt.resize(local_count);
wash::scalar_force_id.resize(local_count);
wash::scalar_force_mass.resize(local_count);
wash::scalar_force_density.resize(local_count);
wash::scalar_force_smoothing_length.resize(local_count);
wash::vector_force_pos_m1_0.resize(local_count);
wash::vector_force_pos_m1_1.resize(local_count);
wash::vector_force_pos_m1_2.resize(local_count);
wash::vector_force_pos_0.resize(local_count);
wash::vector_force_pos_1.resize(local_count);
wash::vector_force_pos_2.resize(local_count);
wash::vector_force_vel_0.resize(local_count);
wash::vector_force_vel_1.resize(local_count);
wash::vector_force_vel_2.resize(local_count);
wash::vector_force_acc_0.resize(local_count);
wash::vector_force_acc_1.resize(local_count);
wash::vector_force_acc_2.resize(local_count);
...
;
        ...
        (*domain).exchangeHalos(std::tie(wash::scalar_force_mass,wash::scalar_force_density,wash::vector_force_vel_0,wash::vector_force_vel_1,wash::vector_force_vel_2,wash::scalar_force_p,wash::scalar_force_c,wash::scalar_force_c11,wash::scalar_force_c12,wash::scalar_force_c13,wash::scalar_force_c22,wash::scalar_force_c23,wash::scalar_force_c33,wash::scalar_force_smoothing_length), s1, s2);
...
(*domain).exchangeHalos(std::tie(wash::scalar_force_smoothing_length,wash::scalar_force_mass,wash::vector_force_vel_0,wash::vector_force_vel_1,wash::vector_force_vel_2), s1, s2);

loop_kernels[0]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 0, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[1]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 1, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
(*domain).exchangeHalos(std::tie(wash::scalar_force_density,wash::scalar_force_p,wash::scalar_force_c), s1, s2);

loop_kernels[2]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 2, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
(*domain).exchangeHalos(std::tie(wash::scalar_force_c11,wash::scalar_force_c12,wash::scalar_force_c13,wash::scalar_force_c22,wash::scalar_force_c23,wash::scalar_force_c33), s1, s2);

loop_kernels[3]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 3, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[4]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 4, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[5]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 5, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[6]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 6, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[7]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 7, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
loop_kernels[8]->exec();
iter_k1 = std::chrono::high_resolution_clock::now();
io.write_timings("kernel_run", 8, diff_ms(iter_k0, iter_k1));
iter_k0 = std::chrono::high_resolution_clock::now();
;
	...
	unsigned count = cstone::findNeighbors(p, 
		wash::vector_force_pos_0.data(), wash::vector_force_pos_1.data(), wash::vector_force_pos_2.data(),wash::scalar_force_smoothing_length.data(),
...
;
    ...
    double eucdist(const Particle& p, const Particle& q) {
        SimulationVecT diff = ((wash::SimulationVecT) {wash::vector_force_pos_0[p], wash::vector_force_pos_1[p], wash::vector_force_pos_2[p]}) - ((wash::SimulationVecT) {wash::vector_force_pos_0[q], wash::vector_force_pos_1[q], wash::vector_force_pos_2[q]});
